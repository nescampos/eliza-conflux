...
This fork is a **simplified example of Eliza focused on executing transactions onchain on Conflux** using [GOAT](https://github.com/goat-sdk/goat-sdk). It's designed specifically for Conflux, focusing on agents that handle complex onchain tasks. By using GOAT for all onchain functionality, this version removes unnecessary plugins for other blockchains and introduces a simpler character to start with that is tailored to doing actions on Conflux.
...
- [Conflux](https://confluxnetwork.org/) - The L2
...
```bash
git clone https://github.com/nescampos/eliza-conflux
```
...
1. This example uses a key pair wallet to connect to Conflux. Save the key and fill in the following in the .env file:
    - `EVM_PRIVATE_KEY=your_private_key`
    - `EVM_PROVIDER_URL=https://evm.confluxscan.io` (more on Conflux EVM RPCs [here](https://doc.confluxnetwork.org/docs/espace/DeveloperQuickstart))

### Running the agent

1. You can now run the agent with the command `pnpm start --character="characters/conflux-hacker.character.json"`
...
### The character
- You can see the definition of your character in the `characters/conflux-hacker.character.json` file.
